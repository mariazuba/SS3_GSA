...
data_path <- "datos_GSA1_SS3.xlsx" # data in SS3 format

#'*###############################################################################*
# DATA FILE ----
#'*###############################################################################*

inputs$dat$Comments <- sprintf("#C %s model generated by GSA1", mod_path)

## General specification ----
inputs$dat$styr<-2003
inputs$dat$endyr<-2021
inputs$dat$Nsexes<-1
inputs$dat$Nages<-6
...
inputs$dat$binwidth<-1
inputs$dat$minimum_size<-10
inputs$dat$maximum_size<-75
...
#inputs$dat$lencomp <- NULL

#'********************
## Age composition ----
#'********************
##'*Specifications* 
inputs$dat$N_agebins<-6
inputs$dat$agebin_vector<-seq(0,5,1)
...
inputs$ctl$Growth_Age_for_L2<-5
#'*------------------------------------------------------*
...
inputs$ctl$F_ballpark_year<-  -inputs$dat$styr
...
inputs$fore$Ydecl<-2003
inputs$fore$Yinit<-2003
#'  
#' #'*------------------------------------------------------*
#' ## Write back to mod_path ----
mod_path_out<-"SS3_model_GSA1"
dir.create(path=mod_path_out, showWarnings = TRUE, recursive = TRUE)
 r4ss::SS_write(inputs, dir = mod_path_out, overwrite = TRUE)
#' 
#' #' #'Ejecutar el modelo ----
#' #' #'*Entramos en el directorio desde la terminal*
system(paste0("cd ",mod_path_out,"/"))
#' #' 
#' #' #'*Permitimos que el ordenador pueda abrir el ejecutable (terminal)*
 r4ss::get_ss3_exe(dir = mod_path_out, version = "v3.30.19")
system(paste0("chmod 755 ",mod_path_out,"/ss3"))
#' #' 
#' #' #'* Ejecutar el modelo SS3 usando la función run_SS_models* ----
 #r4ss::run(dir=mod_path_out, extras = "-nohess",exe="ss3", skipfinished=FALSE, show_in_console =T)
 r4ss::run(dir=mod_path_out,exe="ss3", skipfinished=FALSE, show_in_console =T)
#'  
#'  #'*read the model output and print diagnostic messages*
 replist_merluza <- SS_output(
   dir = mod_path_out,
   verbose = TRUE,
   printstats = TRUE
 )
