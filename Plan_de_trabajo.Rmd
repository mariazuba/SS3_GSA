...
dat1$Nfleets        <-2     #_Nfleets (including surveys)
```
...
type1<-c(1,3)
surveytiming1<-c(-1,1)
units_of_catch1<-c(1,2) 
areas1<-c(1,1) 
need_catch_mult1<-c(0,0)
#---------------------------------------------------------------------------------
# crear data.frame 
...
dat1$lbin_method  <-2     
dat1$binwidth     <-2        
dat1$minimum_size <-10  
dat1$maximum_size <-70   
dat1$use_lencomp  <-0     
```
...
```{r}
len_info_mintailcomp<-rep(-1,2)
len_info_addtocomp<-rep(0.001,2)
len_info_combine_M_F<-rep(0,2)
len_info_CompressBins<-rep(0,2)
len_info_CompError<-rep(0,2)
len_info_ParmSelect<-rep(0,2)
len_info_minsamplesize<-rep(1,2)
#---------------------------------------------------------------------------------
# crear data.frame 
...
row.names(len_info1)<-c("FISHERY", "SURVEY1")
#---------------------------------------------------------------------------------
...
```{r}
# Arreglo de datos
matrix_ageerror<-rbind(rep(-1,6),rep(0.001,6))
#---------------------------------------------------------
dat1$N_ageerror_definitions<-1   #_N_ageerror_definitions
dat1$N_ageerror_definitions
dat1$ageerror<-matrix(matrix_ageerror,nrow=2) #_ageerror_definitions
dat1$ageerror
```
...
```{r}
#------------------------------------------------------------------------------------------
age_info1<-data.frame(mintailcomp=rep(-1,2),
                      addtocomp=rep(0.001,2),
                      combine_M_F=rep(0,2),
                      CompressBins=rep(0,2),
                      CompError=rep(0,2),
                      ParmSelect=rep(0,2),
                      minsamplesize=rep(1,2))

row.names(age_info1)<-c("FISHERY", "SURVEY1")
#---------------------------------------------------------------------------------
...
dat1$Lbin_method <- 1 
#------------------------------------------------------------------------------------------
new_agecomp<-data.frame(Yr=c(2003:2021,2003:2021), 
                                Seas=rep(7,(19*2)), 
                                FltSvy=c(rep(1,19), rep(2,19)),
                                Gender=rep(3,(19*2)), 
                                Part=rep(0,(19*2)),  
                                Ageerr=rep(2,(19*2)), 
                                Lbin_lo=rep(-1,(19*2)), 
...
TUNEFF.DAT<-TUNEFF.DAT[,2:7]
names(TUNEFF.DAT)<-dat_rows_names 

new_agecomp<-cbind(new_agecomp, rbind(CATNUM.DAT,TUNEFF.DAT))
...
#elimino los parámetros que no se utilizaran en el modelo  por el momento
patron_eliminar<-rownames(ctl1$Q_parms)[grep("_SURVEY2",rownames(ctl1$Q_parms))] 
ctl1$Q_parms<- subset(ctl1$Q_parms, !rownames(ctl1$Q_parms) %in%
                          c(patron_eliminar,"Q_extraSD_SURVEY1(2)"))
```
...
```{r}
#--------------------------------------------------
size_selex_types1<-data.frame("Pattern" = 0,
                              "Discard" = 0,
                              "Male"    = 0,
                              "Special" = 0)
...
#--------------------------------------------------
size_selex_types2<-data.frame("Pattern" = 0,
                              "Discard" = 0,
                              "Male"    = 0,
                              "Special" = 0)
...
#--------------------------------------------------
#elimino los parámetros que no se utilizaran en el modelo  por el momento
patron_eliminar<-rownames(ctl1$size_selex_types)[grep("SURVEY2",rownames(ctl1$size_selex_types))] 
ctl1$size_selex_types<- subset(ctl1$size_selex_types, !rownames(ctl1$size_selex_types) %in%
                          c(patron_eliminar))
```
...
#--------------------------------------------------
#elimino los parámetros que no se utilizaran en el modelo  por el momento
patron_eliminar<-rownames(ctl1$age_selex_types)[grep("SURVEY2",rownames(ctl1$age_selex_types))] 
ctl1$age_selex_types<- subset(ctl1$age_selex_types, !rownames(ctl1$age_selex_types) %in%
                          c(patron_eliminar))
...
age_selex_parms3<-data.frame(row.names="AgeSel_P_1_SURVEY1(2)",
                      ...
                      "Block"        = 0,
                      "Block_Fxn"    = 0)
ctl1$age_selex_parms[12,]<-age_selex_parms3
rownames(ctl1$age_selex_parms)[12]<-"AgeSel_P_1_SURVEY1(2)"
#--------------------------------------------------
age_selex_parms4<-data.frame(row.names="AgeSel_P_2_SURVEY1(2)",
                      ...
                      "Block"        = 0,
                      "Block_Fxn"    = 0)
ctl1$age_selex_parms[13,]<-age_selex_parms4
rownames(ctl1$age_selex_parms)[13]<-"AgeSel_P_2_SURVEY1(2)"
#--------------------------------------------------
#elimino los parámetros que no se utilizaran en el modelo  por el momento
patron_eliminar<-rownames(ctl1$age_selex_parms)[c(3:11,14:24)]
ctl1$age_selex_parms<- subset(ctl1$age_selex_parms, !rownames(ctl1$age_selex_parms) %in%
                          c(patron_eliminar))
...
#----------------------------------------------------------------------
ctl1$stddev_reporting_specs <-"#"
ctl1$stddev_reporting_selex <-"#"
ctl1$stddev_reporting_N_at_A <-"#"
...
start1$minyr_sdreport     <-2000
start1$maxyr_sdreport     <-2023
start1$N_STD_yrs          <-0
...
fore1$FirstYear_for_caps_and_allocations   <- 2020
...
wt_flt<-data.frame(do.call(rbind,replicate(2,data_wt,simplify = FALSE))) %>% mutate(type=c(rep("#wt_flt_1",nyears),rep("#wt_flt_2",nyears)))
...
wtatege_f1<-data.frame(Yr=c(rep(years,5)), 
                     Seas=1, 
                     Sex=1, 
                     Bio_Pattern=1,
                     BirthSeas=1,
                     Fleet=c(rep(1,nyears),rep(2,nyears),
                             rep(-2,nyears),
...
###  Escribir archivo de wtatage modificado con la función  `SS_write` para el enfoque de modelación `SS3`
```{r eval=T}
r4ss::SS_writewtatage(mylist = wtat1,
                      dir = here(dirname.simple_mod),
                      file ="wtatage.ss",
                      overwrite = TRUE,
                      verbose = TRUE)
